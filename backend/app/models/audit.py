...
class AuditTemplate(Base):
    __tablename__="audit_templates";__table_args__=(UniqueConstraint("organisation_id","branch_id","name",name="uq_audit_template_scope_name"),)
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uid);organisation_id:Mapped[str]=mapped_column(ForeignKey("organisations.id"),index=True);branch_id:Mapped[str|None]=mapped_column(ForeignKey("branches.id"),index=True,nullable=True)
    name:Mapped[str]=mapped_column(String(180));description:Mapped[str]=mapped_column(Text);category:Mapped[str]=mapped_column(String(80));required_product_count:Mapped[int]=mapped_column(Integer,default=3);require_unique_products:Mapped[bool]=mapped_column(Boolean,default=True);require_photo:Mapped[bool]=mapped_column(Boolean,default=True);require_expiry_date:Mapped[bool]=mapped_column(Boolean,default=True);default_priority:Mapped[str]=mapped_column(String(20),default="MEDIUM");expected_min_duration_seconds:Mapped[int]=mapped_column(Integer,default=60);recurrence_type:Mapped[str]=mapped_column(String(30),default="NONE");active:Mapped[bool]=mapped_column(Boolean,default=True);created_by:Mapped[str]=mapped_column(ForeignKey("users.id"));created_at:Mapped[datetime]=mapped_column(DateTime,default=utc_now);updated_at:Mapped[datetime]=mapped_column(DateTime,default=utc_now,onupdate=utc_now)

class AuditTask(Base):
    __tablename__="audit_tasks"
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uid);organisation_id:Mapped[str]=mapped_column(ForeignKey("organisations.id"),index=True);branch_id:Mapped[str]=mapped_column(ForeignKey("branches.id"),index=True);assignee_id:Mapped[str]=mapped_column(ForeignKey("users.id"),index=True);template_id:Mapped[str|None]=mapped_column(ForeignKey("audit_templates.id"),index=True,nullable=True)
