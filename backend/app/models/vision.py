...
class CameraRule(Base):
    ...
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uid);organisation_id:Mapped[str]=mapped_column(ForeignKey("organisations.id"),index=True);camera_id:Mapped[str]=mapped_column(ForeignKey("cameras.id"),index=True);rule_type:Mapped[str]=mapped_column(String(50));detection_engine:Mapped[str]=mapped_column(String(50),default="OPENCV_RULE_BASED");roi:Mapped[str]=mapped_column(String(120),default="0,0,1,1");threshold:Mapped[float]=mapped_column(Float,default=.3);trigger_frames:Mapped[int]=mapped_column(Integer,default=15);clear_frames:Mapped[int]=mapped_column(Integer,default=30);enabled:Mapped[bool]=mapped_column(Boolean,default=True);current_state:Mapped[str]=mapped_column(String(30),default="CLEAR");last_frame_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True),nullable=True);last_event_id:Mapped[str|None]=mapped_column(String(36),nullable=True);processing_error:Mapped[str|None]=mapped_column(Text,nullable=True);fps_estimate:Mapped[float|None]=mapped_column(Float,nullable=True)
class CameraClipMetadata(Base):
    __tablename__="camera_clip_metadata"
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uid);organisation_id:Mapped[str]=mapped_column(ForeignKey("organisations.id"),index=True);camera_id:Mapped[str]=mapped_column(ForeignKey("cameras.id"),index=True);camera_event_id:Mapped[str]=mapped_column(ForeignKey("camera_events.id"),index=True);frame_path:Mapped[str|None]=mapped_column(String(255),nullable=True);clip_path:Mapped[str|None]=mapped_column(String(255),nullable=True);roi:Mapped[str]=mapped_column(String(120));score:Mapped[float]=mapped_column(Float);frame_number:Mapped[int|None]=mapped_column(Integer,nullable=True);source_timestamp_ms:Mapped[float|None]=mapped_column(Float,nullable=True);engine:Mapped[str]=mapped_column(String(50),default="OPENCV_RULE_BASED");created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utc_now)
