...
def list_news(user:User=Depends(current_user),db:Session=Depends(get_db)):
    return db.scalars(select(News).where(News.organisation_id==market_id(user)).order_by(News.published_at.desc())).all()

@router.get("/news/{news_id}")
def news_detail(news_id:str,user:User=Depends(current_user),db:Session=Depends(get_db)):
    item=db.scalar(select(News).where(News.id==news_id,News.organisation_id==market_id(user)));
