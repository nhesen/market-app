...
export async function selectMarket(id:string){const result=await request<{organisation_id:string;name:string}>("/profile/preferred-market",{method:"PATCH",body:JSON.stringify({organisation_id:id})});await Promise.all([SecureStore.setItemAsync("selected_market_id",id),SecureStore.deleteItemAsync("selected_branch_id")]);return result}
export async function selectedMarketId(){return SecureStore.getItemAsync("selected_market_id")}
export async function recentSearches(){try{return JSON.parse(await SecureStore.getItemAsync("recent_searches")??"[]")as string[]}catch{return[]}}
export async function rememberSearch(value:string){const term=value.trim();if(!term)return;const items=await recentSearches();await SecureStore.setItemAsync("recent_searches",JSON.stringify([term,...items.filter(x=>x.toLowerCase()!==term.toLowerCase())].slice(0,8)))}
export async function clearRecentSearches(){await SecureStore.deleteItemAsync("recent_searches")}
export function mediaUrl(value?:string){if(!value)return undefined;if(/^https?:/.test(value))return value;return serverRoot+value}
...
export const customerApi={
  ...
  discounts:(category="")=>request<any[]>(`/discounts${category?`?category=${encodeURIComponent(category)}`:""}`),discount:(id:string)=>request<any>(`/discounts/${id}`),favourites:()=>request<Product[]>("/favourites/products"),favourite:(id:string)=>request(`/favourites/products/${id}`,{method:"POST"}),unfavourite:(id:string)=>request(`/favourites/products/${id}`,{method:"DELETE"}),
  ...
  me:()=>request<User>("/auth/me"),market:()=>request<{id:string;name:string}>("/market"),updateProfile:(body:unknown)=>request<User>("/profile",{method:"PATCH",body:JSON.stringify(body)}),deleteRequest:(reason:string)=>request("/profile/delete-request",{method:"POST",body:JSON.stringify({reason})}),preferences:()=>request<Record<string,boolean>>("/profile/preferences"),updatePreferences:(body:Record<string,boolean>)=>request<Record<string,boolean>>("/profile/preferences",{method:"PATCH",body:JSON.stringify(body)}),cards:()=>request<any[]>("/loyalty/cards"),transactions:(id:string)=>request<any[]>(`/loyalty/cards/${id}/transactions`),offers:()=>request<any[]>("/loyalty/offers"),offer:(id:string)=>request<any>(`/loyalty/offers/${id}`)
};
export async function uploadAsset(uri:string,type="image/jpeg",onProgress?:(value:number)=>void){
  const token=await SecureStore.getItemAsync("token"),body=new FormData(),extension=type==="video/mp4"?"mp4":type==="image/png"?"png":"jpg";body.append("file",{uri,name:`martiq-evidence.${extension}`,type}as any);
  return new Promise<any>((resolve,reject)=>{const xhr=new XMLHttpRequest();xhr.open("POST",apiRoot+"/uploads");xhr.setRequestHeader("Authorization",`Bearer ${token}`);xhr.upload.onprogress=event=>{if(event.lengthComputable)onProgress?.(event.loaded/event.total)};xhr.onerror=async()=>reject(new Error(await fallbackError("network")));xhr.onload=async()=>{if(xhr.status>=200&&xhr.status<300){onProgress?.(1);try{resolve(JSON.parse(xhr.responseText))}catch{reject(new Error(await fallbackError()))}}else{try{const body=JSON.parse(xhr.responseText);reject(new Error(body.detail??await fallbackError()))}catch{reject(new Error(await fallbackError()))}}};xhr.send(body)});
}
...
export const staffApi={audits:()=>request<any[]>("/staff/audits"),audit:(id:string)=>request<any>(`/staff/audits/${id}`),dashboard:()=>request<any>("/staff/dashboard"),quality:()=>request<any>("/staff/quality-summary"),productByBarcode:(barcode:string)=>request<any>(`/staff/products/barcode/${encodeURIComponent(barcode)}`),reAudits:()=>request<any[]>("/staff/re-audits"),completeReAudit:(id:string,condition:string)=>request<any>(`/staff/re-audits/${id}/complete`,{method:"POST",body:JSON.stringify({condition})}),start:(id:string)=>request<any>(`/staff/audits/${id}/start`,{method:"POST"}),addItem:(id:string,body:unknown)=>request<any>(`/staff/audits/${id}/items`,{method:"POST",body:JSON.stringify(body)}),complete:(id:string)=>request<any>(`/staff/audits/${id}/complete`,{method:"POST"})};
export const adminApi={
  dashboard:()=>request<any>("/admin/dashboard"),incidents:()=>request<any[]>("/admin/incidents"),reports:()=>request<any[]>("/admin/reports"),audits:()=>request<any[]>("/admin/audits"),network:()=>request<any[]>("/admin/network-analytics"),analytics:()=>request<any>("/admin/operational-analytics"),platformHealth:()=>request<any>("/platform/health"),organisations:()=>request<any[]>("/platform/organisations"),tenantUsage:()=>request<any[]>("/platform/tenant-usage")
};
