...
export default function Login() {
  ...
  async function submit() {
    ...
    try {
      ...
      else if (user.role === "HEAD_OFFICE_ADMIN")
          ...
          <View style={s.card}>
            ...
            <Pressable onPress={() => router.push("/register")}>
              <Text style={s.register}>
                {t("noAccount")} <Text style={s.link}>{t("register")}</Text>
              </Text>
            </Pressable>
          </View>
        </ScrollView>
...
const s = StyleSheet.create({
  ...
  title: { fontSize: 29, fontWeight: "900", color: colors.navy, marginTop: 12 },
  subtitle: {
    color: colors.muted,
    textAlign: "center",
    lineHeight: 21,
  ...
  input: { flex: 1, color: colors.navy, fontWeight: "600" },
  ...
  register: { textAlign: "center", color: colors.muted, padding: 8 },
  errorBox: {
    flexDirection: "row",
    gap: 8,
